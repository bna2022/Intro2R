...
1. Geben Sie alle Elemente von `x` aus, die größer als 2 sind.
2. Bestimmen Sie die Positionen aller negativen Werte in `x`.
3. Berechnen Sie die elementweise Summe von `x` und `y`.
4. Berechnen Sie das Skalarprodukt von `x` und `y`.
5. Hängen Sie die beiden Vektoren `x` und `y` zu einem neuen Vektor zusammen.
6. Berechnen Sie den Mittelwert aller positiven Werte aus `x`.
